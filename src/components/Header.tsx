...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-brand-bg/80 backdrop-blur-sm w-full overflow-x-hidden shadow-sm">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-4">
          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
            {navItems.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className={`text-brand-dark text-lg hover:text-brand-yellow transition-colors ${item.label === 'Resources' ? 'flex items-center' : ''}`}
              >
                {item.label}
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden pt-6 pb-8 bg-brand-bg/95">
            <div className="flex flex-col space-y-4">
              {navItems.map((item) => (
                <a key={item.label} href={item.href} className="text-brand-dark text-lg py-2 hover:text-brand-yellow transition-colors" onClick={toggleMenu}>
                  {item.label}
                </a>
              ))}
              <button className="bg-brand-green text-white px-6 py-3 rounded-full hover:bg-opacity-90 transition-all mt-4 w-full">
                Get Consultation
              </button>
            </div>
          </div>
